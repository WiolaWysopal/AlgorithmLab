...
function MergeSortVisualizer() {
  ...
  useEffect(() => {
    const fetchDescription = async () => {
      try {
        const res = await fetch("http://localhost:5000/algorithms/MergeSort");
            ...
            <div
              ...
              className={`d-flex justify-content-center align-items-end ${
                activeStep?.sorted
                  ...
                  : activeStep?.overwrittenIndex === idx
                    ? "bg-info text-dark"
                    : activeStep?.comparing?.includes(idx)
                      ? "bg-warning text-dark"
                      : activeStep?.leftRange?.includes(idx)
                        ? "bg-primary text-white"
                        : activeStep?.rightRange?.includes(idx)
